...
class ResizeableImage(ImageMatrix):
        ...
        for j in range(1, self.height):
            for i in range(self.width):
                if min_energy_table[i, j] == -1:
                    self._calc_min_energy_dp(
                        i, j, min_energy_table, min_ind_table, energy_cache
                    )
    ...
    def _calc_min_energy_dp(
        ...
        i: int,
        j: int,
        min_energy_table: np.ndarray,
        min_ind_table: np.ndarray,
        energy_cache: np.ndarray,
    ) -> None:
        """Compute the min energy of a vertical seam ending at pixel [i, j] using DP."""
        prev_j = j - 1
        left_i, right_i = max(0, i - 1), min(i + 2, self.width)
        min_prev_i = min(
            range(left_i, right_i), key=lambda idx: min_energy_table[idx, prev_j]
        )

        min_energy_table[i, j] = min_energy_table[
            min_prev_i, prev_j
        ] + self._get_cached_energy(i, j, energy_cache)
